...
import { formatCapturedAt, resetDisplay, usageMetaLine, usageWindowTone } from '../usage';
      ...
      {usage.windows.map(window => {
        ...
        return (
          <div key={window.label}>
            ...
            {window.detail && (
              <div className="pl-16 text-[10px] text-fg-5">{window.detail}</div>
            )}
          </div>
        );
      })}
      {usageMetaLine(usage, t) && (
        <div className="mt-0.5 text-[10px] text-fg-5">{usageMetaLine(usage, t)}</div>
      )}
      {capturedAt && (
