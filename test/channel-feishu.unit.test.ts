...
describe('FeishuChannel.editCard', () => {
  ...
  it('returns silently when patch succeeds (code === 0)', async () => {
  ...
  it('throws a feishuEditFailed error when Feishu returns code !== 0 with HTTP 200', async () => {
    const { ch, patchMock } = createTestChannel();
  ...
  it('swallows "not modified" response (content unchanged is a no-op)', async () => {
    const { ch, patchMock } = createTestChannel();
  ...
  it('swallows thrown "not modified" errors from older SDK paths', async () => {
    const { ch, patchMock } = createTestChannel();
  ...
  it('re-throws real network errors with feishuEditFailed tag', async () => {
    const { ch, patchMock } = createTestChannel();
  ...
  it('no-ops when markdown is empty', async () => {
    const { ch, patchMock } = createTestChannel();
    await expect(
