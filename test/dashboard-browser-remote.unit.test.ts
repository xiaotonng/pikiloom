...
// missing" while PIKILOOP_BROWSER_CDP_URL was actually driving a remote browser.
const headlessNoChrome = {
  status: 'chrome_missing' as const,
  profileDir: '/home/piki/.pikiloop/browser/chrome-profile',
...
describe('dashboard browser status — remote CDP mode (issue #16)', () => {
  const prev = process.env.PIKILOOP_BROWSER_CDP_URL;
  beforeEach(() => { delete process.env.PIKILOOP_BROWSER_CDP_URL; });
  afterEach(() => {
    if (prev === undefined) delete process.env.PIKILOOP_BROWSER_CDP_URL;
    else process.env.PIKILOOP_BROWSER_CDP_URL = prev;
  });
  ...
  it('surfaces remote endpoint when enabled, hides it when disabled, and returns null in local mode', async () => {
    ...
    process.env.PIKILOOP_BROWSER_CDP_URL = 'http://chromium:9223';
