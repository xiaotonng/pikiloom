...
describe('dashboard session failure display', () => {
  it('keeps a done-stream error visible even when partial output exists', () => {
  ...
  it('surfaces persisted incomplete run detail after the live stream is gone', () => {
  ...
  it('does not duplicate the persisted failure while live stream UI owns the turn', () => {
    expect(getSessionRunFailureDetail({
      running: false,
      runState: 'incomplete',
      runDetail: 'Timed out.',
