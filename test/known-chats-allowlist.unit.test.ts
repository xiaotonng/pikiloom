...
const ENV_KEYS = [
  'PIKICLAW_CONFIG', 'PIKICLAW_WORKDIR', 'PIKICLAW_ALLOWED_IDS',
...
beforeEach(() => {
  ...
  // real ~/.pikiclaw/setting.json.
  process.env.PIKICLAW_CONFIG = path.join(makeTmpDir('known-allow-config-'), 'setting.json');
  process.env.PIKICLAW_WORKDIR = makeTmpDir('known-allow-work-');
  process.env.TELEGRAM_BOT_TOKEN = 'test-token';
  process.env.FEISHU_APP_ID = 'test-app';
  process.env.FEISHU_APP_SECRET = 'test-secret';
  delete process.env.PIKICLAW_ALLOWED_IDS;
...
describe('known chats never pollute the allowlist (regression for #22)', () => {
  ...
  it('Telegram: an explicit allowlist is preserved without absorbing known chats', () => {
    recordKnownChatId('telegram', '12345');
    process.env.PIKICLAW_ALLOWED_IDS = '999';
